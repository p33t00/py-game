...
class TestIntelligenceHigh:
    ...
    @pytest.mark.parametrize('should_roll_args', [
    ...
    def test_should_roll(self, intelligence, should_roll_args):
        assert intelligence.should_roll( \
            should_roll_args[0], \
            should_roll_args[1], \
            should_roll_args[2], \
            should_roll_args[3],
    ...
    def test_is_player_final_roll(self, intelligence, participant_score):
        assert intelligence.is_player_final_roll(participant_score[0]) == participant_score[1]
        
    """0 -> turn_total_score, 1 -> bot_score, 2 -> result"""
    @pytest.mark.parametrize('participant_score', [(70, 4, False),(90, 4, True),(80, 18, True)])
    def test_is_bot_final_roll(self, intelligence, participant_score):
        assert intelligence.is_bot_final_roll(participant_score[0], participant_score[1]) \
            == participant_score[2]
    ...
    def test_is_optimal_turn_score(self, intelligence, turn_total_points):
        assert intelligence.is_optimal_turn_score(turn_total_points[0]) == turn_total_points[1]
