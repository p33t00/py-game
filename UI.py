...
class UI(Cmd):
    ...
    def do_stop(self, arg):
        ...
        # check if player stopped or got 1
        if dice.get_turn_total_score() > 0:
            self.finalize_turn(self.get_game(), dice, player)
        
        # deal with this line
        print(f"{player.get_name()} total score is {player.get_total_points()}")

        # Stopping the turn and pass it to next player
        print("Now it's Computer turn to roll...")
        self.bot_play(dice, bot, player.get_total_points(), self.get_game().get_winner_score())
